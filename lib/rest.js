...
function k8create(socket, restOptions, api, type, data){
...
function k8delete(socket, restOptions, api, type, id){
...
function k8update(socket, restOptions, api, type, id, data){
    url="https://" + restOptions.hostip +"/api/" + api + "/" + type + "/" + id;
...
function handleRestReturn(socket, err, resp, body, type, operation, id){
    if (err){
        ...
        if (resp.statusCode >= 200 && resp.statusCode < 300){
            var ret={'status': resp.statusCode, 'type': type, 'operation': operation, 'text': "Success: " + type + " " + id + " "+ operation + " on k8s", "responseObj": resp, 'bodyObj': body};
            cli.info (ret.text);
